...
def create_key_phrases(row):
    """Uses AWS Comprehend to Create Sentiments on a DataFrame"""

    LOG.info(f"Processing {row}")
    comprehend = boto3.client(service_name='comprehend')
    payload = comprehend.detect_key_phrases(Text=row, LanguageCode='en')
    LOG.debug(f"Found KeyPhrases: {payload}")
    key_phrases = payload['KeyPhrases']
    return key_phrases

def apply_key_phrases(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Create Key Phrases"""
    df['KeyPhrases'] = df[column].apply(create_key_phrases)
    return df
...
def lambda_handler(event, context):
    ...
    #Perform Key Phrases Analysis
    df = apply_key_phrases(df)
    LOG.info(f"KeyPhrases from FANG companies: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="realfangsentiment", name=names)